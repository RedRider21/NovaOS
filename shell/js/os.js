...
const OS = (() => {
  ...
  async function quickTap(el, native) {
    ...
    if (isSensor && native) {
      ...
      //
      // ATTENZIONE, l'`await` è obbligatorio. Lo stesso comando torna un booleano
      // sincrono sotto WebView (@JavascriptInterface) e una Promise sotto GeckoView
      // (richiesta/risposta sul ponte a messaggi): senza l'await il confronto non
      // combaciava mai, `applied` restava `null` per ogni interruttore e il ramo che
      // ridisegna non si prendeva più. Await su un valore non-Promise restituisce il
      // valore stesso, quindi la riga è corretta in entrambi i contenitori — ed è
      // l'unica forma che non chiede alla shell di sapere dove sta girando.
  ...
  async function quickAct(act, k, el) {
    if (act === "torch") {
      ...
      // L'await serve qui più che altrove: la torcia non ha un broadcast che rimedi,
      // quindi senza di esso l'interruttore restava spento con la torcia accesa.
